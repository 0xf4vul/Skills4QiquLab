...
### Phase 6 — Acquire & count 30D activity

先取得目标平台完整时间线 → 判断 Timeline status → 逐条记录直接帖子 URL / post ID、发布时间、类型与主题 → 完成完整性检查 → 仅在 30D_VERIFIED 时汇总精确总量；否则保持 Unverified / Partial。
...
**10. 其他平台的文章、博客、LinkedIn 帖子、视频等可以作为身份或内容发现证据，但不得转换成目标平台的 30D 帖子记录。**
